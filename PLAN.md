...
| Pricing shape (2026-09-05) | **One purchase per event**, through Stripe Checkout | The only shape that passes the market-anger checklist — a purchase, not a subscription; nothing metered mid-event; the limit is one sentence — and the only one the schema already carries (the per-event window column). The days, the AUD price and the GST stance are settled in the sitting that builds Stripe, so the Price object is created once. |
| Retention anchor (2026-09-06) | **The window counts from the first photo**, not from event creation | It is the one part of the pricing decision that cannot be revised afterwards: the window is a promise made to a guest at the moment they consent — the review screen's consent line, the album header — so once a stranger has run an event it has been made. Anchored to creation, a host who set a wedding up four weeks early had already burned a month of it and the consent line named a date that was running down before anyone arrived. Settled ahead of the Stripe sitting for that reason; only the *number* of days waits for it. |
| Guest takedown (2026-09-06) | **No guest-facing control.** A guest asks the host, or writes to the support inbox and an admin actions it | The mechanism already exists and is operated by a person: a host deletes a session from the album, and an admin can delete any of them. A guest-facing button would be *unsafe* before item 12's device token — photo rows carry no guest identifier, so the only thing that could gate it is the album link, which every guest at the event holds, and it would let anyone delete anyone's photos. A documented manual path satisfies the access-and-correction obligation; the album's footer (26) is what gives a guest who cannot reach the host somewhere to write. |
| Album privacy vs the files (2026-09-06) | The setting gates the **album page**, never the image routes — and the booth's consent note says so | The routes stay session-free and immutably cached (Phill's call, 2026-08-31), so a photo URL opens for whoever holds it. The booth used to tell a guest sharing into a hidden album that "only the host can see it", which is more privacy than the design delivers. The promise moved to match the code: an extra consent line, true under all three settings, reads "Photo links work for anyone who has them". A published privacy policy cannot describe access control while a screen describes it differently. |
...
2. ~~**Per-event branding**~~ — done: owner picks a strip colour theme (Midnight/Blush/Forest/
   ...
   **instead of the caption** (one or the other); a remove option clears it. The live preview shows
   it too. Owners can also **upload a background** — one image behind the whole strip, cover-cropped
   to it, served at `GET /e/{code}/background`, with the same replace/remove pipeline (`applyImage`
...
- **Palette**: ink `#0B0B10` / ivory `#F4F2ED`, accent blue `#3A86FF` (the one CTA colour, with a
  ...
  `strip-theme.ts` — they are the owner's choice, not the app's chrome. **A background does not
  retire the theme**: it is the ground a transparent PNG tints, and it still inks the caption over
  whatever the host's artwork turns out to be.
